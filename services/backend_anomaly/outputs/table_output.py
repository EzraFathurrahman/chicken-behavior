...
class TableOutput:
    ...
    def sort_by_object_id_and_frame(self, file_path):
    ...
    def colorline(self, x, y, z=None, cmap=plt.get_cmap('copper'), norm=plt.Normalize(0.0, 1.0),
    ...
    def plot(self, file_path, i, session_id):
        data = file_path
        N = 10
        np.random.seed(101)
        x = data["cX"]
        ...
        self.colorline(x, y, z, cmap=plt.get_cmap('plasma'), linewidth=2)
        plt.colorbar(col, format="%d")

        plt.savefig("static/temp/{}_{}.png".format(session_id, i))

    def split_object_id(self, file_path):
        df = file_path
    ...
    def make_plot(self):
        self.ids, dataframes = self.split_object_id(self.sort_by_object_id_and_frame(self.contents))
        for i in self.ids:
            dataframes[i] = dataframes[i].reset_index().drop(["index"], axis=1)
            self.plot(dataframes[i], i, self.session_id)
